...
def add_gaussian_noise(image: np.ndarray, sigma: float = 25.0) -> np.ndarray:
    ...
    Returns:
        noisy_image: The image with added noise, clipped to [0, 255]
    """
...
def add_salt_and_pepper(image: np.ndarray, prob: float = 0.05) -> np.ndarray:
    ...
    Args:
        image: The input image as a 2D numpy array
        prob:  Probability of a pixel being affected by noise
    Returns:
...
def add_speckle_noise(image: np.ndarray, sigma: float = 0.1) -> np.ndarray:
     ...
     Args:
         image: The input image as a 2D numpy array
         sigma: Standard deviation of the speckle noise
     Returns:
         noisy_image: The image with added noise, clipped to [0, 255]
     """
